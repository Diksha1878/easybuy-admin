...
<table class="table table-striped table-bordered table-hover" id="combotable">
	...
	<tbody>
		<?php 
			$rows = get_row($conn, "select  * from products where name like '% $key%' or keyword like '% $key%' order by id desc");
			if(count($rows)==0){
			...
			<tr>
				...
				<?php 
					...
					$rows13=get_row($conn, "select * from cat where id='{$row['cat_id']}'");
				?>
				<td><?php echo $rows12[0]['name']; ?></td>
				...
				<td colspan="6" style="padding:0">
					<div>
						<table style="background:#dcdcdc;margin-bottom:0;" class="table table-bordered">
							...
							<tbody>
								<?php
									$rows16=get_row($conn, "select * from products_item where pid='{$row['id']}'");
									...
									<tr>
										...
										<td>
											<?php if(isset($_SESSION['combo_list'][$row16['id']])){ ?>
												<script>
											...
											<input <?php if(isset($_SESSION['combo_list'][$row16['id']])){ echo 'checked'; } ?> onclick="addcombo(<?php echo $row16['id']; ?>,<?php echo $row['id']; ?>)" type="checkbox">
											&nbsp;Add to combo
										</td>
									</tr>
								<?php } ?>
