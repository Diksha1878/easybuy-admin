	...
	if($_POST['status']=='save'){
		if(!isset($_SESSION['combo_list'])){
			...
			if(count($_SESSION['combo_list'])>1){
				...
				$rows3 = get_row($conn, "select * from combo where combo_code='{$combo_id}' or name='{$_POST['comboname']}' LIMIT 1");
				if(count($rows3)==0){
					...
					$conn->query("insert into combo(id,name,combo_code,user_id,role,price,date) values('$Maxid','$combo_name','$combo_id','{$_SESSION['id']}','{$_SESSION['role']}','$total','$date1')");
					foreach($_SESSION['combo_list'] as $id){
